...
void pickup::start()
{
	...
	AL::ALValue config = AL::ALValue::array(AL::ALValue::array("LeftStepHeight", 0.03), AL::ALValue::array("RightStepHeight", 0.03), AL::ALValue::array("LeftMaxStepX", 0.05), AL::ALValue::array("RightMaxStepX", 0.05));
	AL::ALValue configMoveto = AL::ALValue::array(AL::ALValue::array("StepHeight", 0.02), AL::ALValue::array("MaxStepX", 0.04));
	...
	int upCloseToTarget = 180;   //判断上边缘扔垃圾
	...
	// 	imshow("image", image);		   //显示图片
	// 	waitKey(50);				   //等待5秒
	// 	imgSVM.SVMFind(1, image);
	// 	//cout << "8888888888888888888"<<endl;													  //得到中心坐标和左下角y坐标
	...
	while (true)
	{
		...
		imshow("image", image);		   //显示图片
		waitKey(50);				   //等待5秒
		imgSVM.SVMFind(2, image);	  //得到中心坐标和左下角y坐标
	...
	while (true)
	{
		...
		if (imgSVM.up_y() > upCloseToTarget)
		{
			motionProxy.moveToward(0, 0, 0);
			break;
		}
