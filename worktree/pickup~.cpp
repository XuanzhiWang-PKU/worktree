...
void pickup::start()
{
	...
	postureProxy.goToPosture("Stand", 0.5);		//0.5代表速度 原来是standinit 但是要看到手
	...
	camProxy.setParameter(1, 12, 0); //关掉自动白平衡////////////////
	...
	while (true)
	{
		...
		waitKey(50);
							//等待5秒
		find(1, image);					//得到中心坐标和左下角y坐标
		if (down_y > 0 && (area > 800)) //判断是否看到垃圾
		{
			cout << "看到垃圾了" << endl;
			break;
		}
	...
	while (true)
	{
		...
		waitKey(50);													//等待5秒
		find(1, image);													//得到中心坐标和左下角y坐标
		if (center_x < (160 + pickBall) && center_x > (160 - pickBall)) //如果在中间离得近
			motionProxy.moveToward(0.6, 0, 0);							//0.6为速度
		if (center_x < (160 - pickBall))								//如果在左边离得近
			motionProxy.moveToward(0.3, 0.15, 0);
		if (center_x > (160 + pickBall)) //如果在右边离得近
			motionProxy.moveToward(0.3, -0.15, 0);
		if (down_y > closetoballbot && center_x > (160 - pickBall * 2) && center_x < (160 + pickBall * 2)) //如果可以捡到
		{
			motionProxy.moveToward(0, 0, 0);
			break;
		}
		if (down_y > closetoballbot && center_x < (160 - pickBall))
		{
	...
	while (true)
	{
		...
		{
			cout << "下边缘y： " << down_y << endl;

			break;
		}
		//motionProxy.moveToward(0.6, 0, 0);//感觉还行
		motionProxy.moveToward(0.5, 0, 0, config); //0.03的还行 走得大一点 要抖 0.04要摔///
	}
	...
	while (true)
	{
		...
		//waitKey(50);
		find(2, image);													  //得到中心坐标和左下角y坐标
		if (center_x < (160 + throwBall) && center_x > (160 - throwBall)) //如果在中间离得远
			motionProxy.moveToward(0.4, 0, 0, config);
		if (center_x < (160 - throwBall)) //如果在左边离得远
		{
			motionProxy.moveToward(0.2, 0.5, 0, config);
			cout << "我在向左走" << endl;
		}
		if (center_x > (160 + throwBall)) //如果在右边离得远
		{
			motionProxy.moveToward(0.2, -0.5, 0, config);
			cout << "我在向右走" << endl;
		}

		if (center_y > closeToTarget && std::fabs(center_x - 160.f) <= throwBall) //如果可以放下
		{

			break;
		}
		if (center_y > closeToTarget && std::fabs(center_x - 160.f) > throwBall)
		{
			if (center_y > 160 + throwBall)
				motionProxy.moveToward(0, -0.5, 0);
			else if (center_y < 160 - throwBall)
				motionProxy.moveToward(0, 0.5, 0);
		}
	}
	cout << "第三阶段开始了*************************************************************************" << endl;
...
void pickup::find(int num, Mat image)
{
	...
	{
		//hsv
		Mat result1_hsv = imgProcess(image, sample1, 0.05f, 1); //threshold 0.05~0.09
		//rgb
		ColorHistogram h;
		sample1 = h.colorReduce(sample1, 40); //reduce 100~200
		image = h.colorReduce(image, 40);
		Mat result1_rgb = imgProcess(image, sample1, 0.05f, 0); //RGB
	...
	{
		///hsv
		Mat result2_hsv = imgProcess(image, sample2, 0.056f, 1);
		///rgb
		ColorHistogram h;
		sample2 = h.colorReduce(sample2, 69);
		image = h.colorReduce(image, 69);
		Mat result2_rgb = imgProcess(image, sample2, 0.029f, 0);
	...
	{
		///hsv
		Mat result2_hsv = imgProcess(image, sample3, 0.088f, 1);
		///rgb
		ColorHistogram h;
		sample3 = h.colorReduce(sample3, 68);
		image = h.colorReduce(image, 68);
		Mat result2_rgb = imgProcess(image, sample3, 0.079f, 0);
		Mat result2_j = result2_hsv & result2_rgb;
		Mat result2_b = result2_hsv | result2_rgb; // "&"  "|"
		bimg = result2_j;
...
{ //单手捡垃圾
	...
	times[0][0] = 3.12;
	keys[0][0] = AL::ALValue::array(0.0183661, AL::ALValue::array(3, -1.04, 0), AL::ALValue::array(3, 0.8, 0));
	times[0][1] = 5.52;
	...
	times[0][3] = 9.52;
	keys[0][3] = AL::ALValue::array(-0.628982, AL::ALValue::array(3, -0.733333, 0), AL::ALValue::array(3, 0.0533333, 0));
	times[0][4] = 9.68;
	keys[0][4] = AL::ALValue::array(-0.628982, AL::ALValue::array(3, -0.0533333, 0), AL::ALValue::array(3, 1.2, 0));
	times[0][5] = 13.28;
	keys[0][5] = AL::ALValue::array(-0.635118, AL::ALValue::array(3, -1.2, 0), AL::ALValue::array(3, 1.65333, 0));
	times[0][6] = 18.24;
	keys[0][6] = AL::ALValue::array(-0.016916, AL::ALValue::array(3, -1.65333, 0), AL::ALValue::array(3, 0, 0));
	...
	times[1][0] = 3.12;
	keys[1][0] = AL::ALValue::array(0.00916195, AL::ALValue::array(3, -1.04, 0), AL::ALValue::array(3, 0.8, 0));
	times[1][1] = 5.52;
	...
	times[1][3] = 9.52;
	keys[1][3] = AL::ALValue::array(0.0168321, AL::ALValue::array(3, -0.733333, 0), AL::ALValue::array(3, 0.0533333, 0));
	times[1][4] = 9.68;
	keys[1][4] = AL::ALValue::array(0.0168321, AL::ALValue::array(3, -0.0533333, 0), AL::ALValue::array(3, 1.2, 0));
	times[1][5] = 13.28;
	keys[1][5] = AL::ALValue::array(0.0152981, AL::ALValue::array(3, -1.2, 0.00107526), AL::ALValue::array(3, 1.65333, -0.00148146));
	times[1][6] = 18.24;
	keys[1][6] = AL::ALValue::array(0.00916195, AL::ALValue::array(3, -1.65333, 0), AL::ALValue::array(3, 0, 0));
	...
	times[2][0] = 3.12;
	keys[2][0] = AL::ALValue::array(-1.18944, AL::ALValue::array(3, -1.04, 0), AL::ALValue::array(3, 0.8, 0));
	times[2][1] = 5.52;
	...
	times[2][3] = 9.52;
	keys[2][3] = AL::ALValue::array(-0.455641, AL::ALValue::array(3, -0.733333, 0), AL::ALValue::array(3, 0.0533333, 0));
	times[2][4] = 9.68;
	keys[2][4] = AL::ALValue::array(-0.455641, AL::ALValue::array(3, -0.0533333, 0), AL::ALValue::array(3, 1.2, 0));
	times[2][5] = 13.28;
	keys[2][5] = AL::ALValue::array(-0.780848, AL::ALValue::array(3, -1.2, 0), AL::ALValue::array(3, 1.65333, 0));
	times[2][6] = 18.24;
	keys[2][6] = AL::ALValue::array(-0.352862, AL::ALValue::array(3, -1.65333, 0), AL::ALValue::array(3, 0, 0));
	...
	times[3][0] = 3.12;
	keys[3][0] = AL::ALValue::array(0.023052, AL::ALValue::array(3, -1.04, 0), AL::ALValue::array(3, 0.8, 0));
	times[3][1] = 5.52;
	...
	times[3][3] = 9.52;
	keys[3][3] = AL::ALValue::array(0.153442, AL::ALValue::array(3, -0.733333, 0), AL::ALValue::array(3, 0.0533333, 0));
	times[3][4] = 9.68;
	keys[3][4] = AL::ALValue::array(0.153442, AL::ALValue::array(3, -0.0533333, 0), AL::ALValue::array(3, 1.2, 0));
	times[3][5] = 13.28;
	keys[3][5] = AL::ALValue::array(0.154976, AL::ALValue::array(3, -1.2, 0), AL::ALValue::array(3, 1.65333, 0));
	times[3][6] = 18.24;
	keys[3][6] = AL::ALValue::array(0.00157595, AL::ALValue::array(3, -1.65333, 0), AL::ALValue::array(3, 0, 0));
	...
	times[4][0] = 3.12;
	keys[4][0] = AL::ALValue::array(-0.987855, AL::ALValue::array(3, -1.04, 0), AL::ALValue::array(3, 0.8, 0));
	times[4][1] = 5.52;
	...
	times[4][3] = 9.52;
	keys[4][3] = AL::ALValue::array(-0.375789, AL::ALValue::array(3, -0.733333, -0.0276471), AL::ALValue::array(3, 0.0533333, 0.0020107));
	times[4][4] = 9.68;
	keys[4][4] = AL::ALValue::array(-0.361981, AL::ALValue::array(3, -0.0533333, -0.000920428), AL::ALValue::array(3, 0.48, 0.00828386));
	times[4][5] = 11.12;
	keys[4][5] = AL::ALValue::array(-0.348176, AL::ALValue::array(3, -0.48, 0), AL::ALValue::array(3, 0.72, 0));
	times[4][6] = 13.28;
	keys[4][6] = AL::ALValue::array(-0.357381, AL::ALValue::array(3, -0.72, 0), AL::ALValue::array(3, 0.0133333, 0));
	times[4][7] = 13.32;
	keys[4][7] = AL::ALValue::array(-0.357381, AL::ALValue::array(3, -0.0133333, 0), AL::ALValue::array(3, 1.64, 0));
	times[4][8] = 18.24;
	keys[4][8] = AL::ALValue::array(-1.0078, AL::ALValue::array(3, -1.64, 0), AL::ALValue::array(3, 0, 0));
	...
	times[5][0] = 3.12;
	keys[5][0] = AL::ALValue::array(-1.37144, AL::ALValue::array(3, -1.04, 0), AL::ALValue::array(3, 0.8, 0));
	times[5][1] = 5.52;
	...
	times[5][3] = 9.52;
	keys[5][3] = AL::ALValue::array(0.170232, AL::ALValue::array(3, -0.733333, 0), AL::ALValue::array(3, 0.0533333, 0));
	times[5][4] = 9.68;
	keys[5][4] = AL::ALValue::array(0.179436, AL::ALValue::array(3, -0.0533333, -0.000170461), AL::ALValue::array(3, 0.48, 0.00153415));
	times[5][5] = 11.12;
	keys[5][5] = AL::ALValue::array(0.18097, AL::ALValue::array(3, -0.48, 0), AL::ALValue::array(3, 0.72, 0));
	times[5][6] = 13.28;
	keys[5][6] = AL::ALValue::array(0.16563, AL::ALValue::array(3, -0.72, 0), AL::ALValue::array(3, 0.0133333, 0));
	times[5][7] = 13.32;
	keys[5][7] = AL::ALValue::array(0.16563, AL::ALValue::array(3, -0.0133333, 0), AL::ALValue::array(3, 1.64, 0));
	times[5][8] = 18.24;
	keys[5][8] = AL::ALValue::array(-1.33922, AL::ALValue::array(3, -1.64, 0), AL::ALValue::array(3, 0, 0));
	...
	times[6][0] = 3.12;
	keys[6][0] = AL::ALValue::array(0.246, AL::ALValue::array(3, -1.04, 0), AL::ALValue::array(3, 0.8, 0));
	times[6][1] = 5.52;
	...
	times[6][3] = 9.52;
	keys[6][3] = AL::ALValue::array(0.88, AL::ALValue::array(3, -0.733333, 0), AL::ALValue::array(3, 0.0533333, 0));
	times[6][4] = 9.68;
	keys[6][4] = AL::ALValue::array(1, AL::ALValue::array(3, -0.0533333, 0), AL::ALValue::array(3, 0.48, 0));
	times[6][5] = 11.12;
	keys[6][5] = AL::ALValue::array(0.8784, AL::ALValue::array(3, -0.48, 0), AL::ALValue::array(3, 0.72, 0));
	times[6][6] = 13.28;
	keys[6][6] = AL::ALValue::array(0.9932, AL::ALValue::array(3, -0.72, 0), AL::ALValue::array(3, 0.0133333, 0));
	times[6][7] = 13.32;
	keys[6][7] = AL::ALValue::array(0.9932, AL::ALValue::array(3, -0.0133333, 0), AL::ALValue::array(3, 1.64, 0));
	times[6][8] = 18.24;
	keys[6][8] = AL::ALValue::array(0.2624, AL::ALValue::array(3, -1.64, 0), AL::ALValue::array(3, 0, 0));
	...
	times[7][0] = 3.12;
	keys[7][0] = AL::ALValue::array(-0.569072, AL::ALValue::array(3, -1.04, 0), AL::ALValue::array(3, 0.8, 0));
	times[7][1] = 5.52;
	...
	times[7][3] = 9.52;
	keys[7][3] = AL::ALValue::array(-1.46953, AL::ALValue::array(3, -0.733333, 0), AL::ALValue::array(3, 0.0533333, 0));
	times[7][4] = 9.68;
	keys[7][4] = AL::ALValue::array(-1.46953, AL::ALValue::array(3, -0.0533333, 0), AL::ALValue::array(3, 1.2, 0));
	times[7][5] = 13.28;
	keys[7][5] = AL::ALValue::array(-0.538392, AL::ALValue::array(3, -1.2, -0.0656897), AL::ALValue::array(3, 1.65333, 0.0905058));
	times[7][6] = 18.24;
	keys[7][6] = AL::ALValue::array(-0.447886, AL::ALValue::array(3, -1.65333, 0), AL::ALValue::array(3, 0, 0));
	...
	times[8][0] = 3.12;
	keys[8][0] = AL::ALValue::array(0.039926, AL::ALValue::array(3, -1.04, 0), AL::ALValue::array(3, 0.8, 0));
	times[8][1] = 5.52;
	...
	times[8][3] = 9.52;
	keys[8][3] = AL::ALValue::array(-0.274544, AL::ALValue::array(3, -0.733333, 0), AL::ALValue::array(3, 0.0533333, 0));
	times[8][4] = 9.68;
	keys[8][4] = AL::ALValue::array(-0.274544, AL::ALValue::array(3, -0.0533333, 0), AL::ALValue::array(3, 1.2, 0));
	times[8][5] = 13.28;
	keys[8][5] = AL::ALValue::array(-0.049046, AL::ALValue::array(3, -1.2, -0.0378551), AL::ALValue::array(3, 1.65333, 0.0521559));
	times[8][6] = 18.24;
	keys[8][6] = AL::ALValue::array(0.00310993, AL::ALValue::array(3, -1.65333, 0), AL::ALValue::array(3, 0, 0));
	...
	times[9][0] = 3.12;
	keys[9][0] = AL::ALValue::array(-0.612024, AL::ALValue::array(3, -1.04, 0), AL::ALValue::array(3, 0.8, 0));
	times[9][1] = 5.52;
	...
	times[9][3] = 9.52;
	keys[9][3] = AL::ALValue::array(-1.13665, AL::ALValue::array(3, -0.733333, 0), AL::ALValue::array(3, 0.0533333, 0));
	times[9][4] = 9.68;
	keys[9][4] = AL::ALValue::array(-1.13665, AL::ALValue::array(3, -0.0533333, 0), AL::ALValue::array(3, 1.2, 0));
	times[9][5] = 13.28;
	keys[9][5] = AL::ALValue::array(-1.12131, AL::ALValue::array(3, -1.2, -0.0153397), AL::ALValue::array(3, 1.65333, 0.0211347));
	times[9][6] = 18.24;
	keys[9][6] = AL::ALValue::array(-0.00762796, AL::ALValue::array(3, -1.65333, 0), AL::ALValue::array(3, 0, 0));
	...
	times[10][0] = 3.12;
	keys[10][0] = AL::ALValue::array(2.10461, AL::ALValue::array(3, -1.04, 0), AL::ALValue::array(3, 0.8, 0));
	times[10][1] = 5.52;
	...
	times[10][3] = 9.52;
	keys[10][3] = AL::ALValue::array(2.11075, AL::ALValue::array(3, -0.733333, 0), AL::ALValue::array(3, 0.0533333, 0));
	times[10][4] = 9.68;
	keys[10][4] = AL::ALValue::array(2.11075, AL::ALValue::array(3, -0.0533333, 0), AL::ALValue::array(3, 1.2, 0));
	times[10][5] = 13.28;
	keys[10][5] = AL::ALValue::array(1.98955, AL::ALValue::array(3, -1.2, 0.121196), AL::ALValue::array(3, 1.65333, -0.166981));
	times[10][6] = 18.24;
	keys[10][6] = AL::ALValue::array(0.70253, AL::ALValue::array(3, -1.65333, 0), AL::ALValue::array(3, 0, 0));
	...
	times[11][0] = 3.12;
	keys[11][0] = AL::ALValue::array(1.42504, AL::ALValue::array(3, -1.04, 0), AL::ALValue::array(3, 0.8, 0));
	times[11][1] = 5.52;
	...
	times[11][3] = 9.52;
	keys[11][3] = AL::ALValue::array(0.377323, AL::ALValue::array(3, -0.733333, -0.0276466), AL::ALValue::array(3, 0.0533333, 0.00201066));
	times[11][4] = 9.68;
	keys[11][4] = AL::ALValue::array(0.401866, AL::ALValue::array(3, -0.0533333, -0.00163625), AL::ALValue::array(3, 0.48, 0.0147262));
	times[11][5] = 11.12;
	keys[11][5] = AL::ALValue::array(0.42641, AL::ALValue::array(3, -0.48, -0.0049088), AL::ALValue::array(3, 0.72, 0.00736319));
	times[11][6] = 13.28;
	keys[11][6] = AL::ALValue::array(0.438682, AL::ALValue::array(3, -0.72, 0), AL::ALValue::array(3, 0.0133333, 0));
	times[11][7] = 13.32;
	keys[11][7] = AL::ALValue::array(0.438682, AL::ALValue::array(3, -0.0133333, 0), AL::ALValue::array(3, 1.64, 0));
	times[11][8] = 18.24;
	keys[11][8] = AL::ALValue::array(1.38363, AL::ALValue::array(3, -1.64, 0), AL::ALValue::array(3, 0, 0));
	...
	times[12][0] = 3.12;
	keys[12][0] = AL::ALValue::array(0.288349, AL::ALValue::array(3, -1.04, 0), AL::ALValue::array(3, 0.8, 0));
	times[12][1] = 5.52;
	...
	times[12][3] = 9.52;
	keys[12][3] = AL::ALValue::array(-0.093616, AL::ALValue::array(3, -0.733333, 0), AL::ALValue::array(3, 0.0533333, 0));
	times[12][4] = 9.68;
	keys[12][4] = AL::ALValue::array(-0.036858, AL::ALValue::array(3, -0.0533333, -0.00102269), AL::ALValue::array(3, 0.48, 0.00920417));
	times[12][5] = 11.12;
	keys[12][5] = AL::ALValue::array(-0.0276539, AL::ALValue::array(3, -0.48, 0), AL::ALValue::array(3, 0.72, 0));
	times[12][6] = 13.28;
	keys[12][6] = AL::ALValue::array(-0.047596, AL::ALValue::array(3, -0.72, 0), AL::ALValue::array(3, 0.0133333, 0));
	times[12][7] = 13.32;
	keys[12][7] = AL::ALValue::array(-0.047596, AL::ALValue::array(3, -0.0133333, 0), AL::ALValue::array(3, 1.64, 0));
	times[12][8] = 18.24;
	keys[12][8] = AL::ALValue::array(0.294486, AL::ALValue::array(3, -1.64, 0), AL::ALValue::array(3, 0, 0));
	...
	times[13][0] = 3.12;
	keys[13][0] = AL::ALValue::array(-0.00771189, AL::ALValue::array(3, -1.04, 0), AL::ALValue::array(3, 0.8, 0));
	times[13][1] = 5.52;
	keys[13][1] = AL::ALValue::array(-1.62148, AL::ALValue::array(3, -0.8, 0.0470424), AL::ALValue::array(3, 0.6, -0.0352818));
	times[13][2] = 7.32;
	keys[13][2] = AL::ALValue::array(-1.65676, AL::ALValue::array(3, -0.6, 0.0294524), AL::ALValue::array(3, 0.733333, -0.0359974));
	times[13][3] = 9.52;
	keys[13][3] = AL::ALValue::array(-1.81783, AL::ALValue::array(3, -0.733333, 0), AL::ALValue::array(3, 0.0533333, 0));
	times[13][4] = 9.68;
	keys[13][4] = AL::ALValue::array(-1.79022, AL::ALValue::array(3, -0.0533333, -0.00132936), AL::ALValue::array(3, 0.48, 0.0119642));
	times[13][5] = 11.12;
	keys[13][5] = AL::ALValue::array(-1.77795, AL::ALValue::array(3, -0.48, 0), AL::ALValue::array(3, 0.72, 0));
	times[13][6] = 13.28;
	keys[13][6] = AL::ALValue::array(-1.78256, AL::ALValue::array(3, -0.72, 0), AL::ALValue::array(3, 0.0133333, 0));
	times[13][7] = 13.32;
	keys[13][7] = AL::ALValue::array(-1.78256, AL::ALValue::array(3, -0.0133333, 0), AL::ALValue::array(3, 1.64, 0));
	times[13][8] = 18.24;
	keys[13][8] = AL::ALValue::array(-0.0537319, AL::ALValue::array(3, -1.64, 0), AL::ALValue::array(3, 0, 0));
	...
	times[14][0] = 3.12;
	keys[14][0] = AL::ALValue::array(-1.18421, AL::ALValue::array(3, -1.04, 0), AL::ALValue::array(3, 0.8, 0));
	times[14][1] = 5.52;
	...
	times[14][3] = 9.52;
	keys[14][3] = AL::ALValue::array(-0.51845, AL::ALValue::array(3, -0.733333, 0), AL::ALValue::array(3, 0.0533333, 0));
	times[14][4] = 9.68;
	keys[14][4] = AL::ALValue::array(-0.51845, AL::ALValue::array(3, -0.0533333, 0), AL::ALValue::array(3, 1.2, 0));
	times[14][5] = 13.28;
	keys[14][5] = AL::ALValue::array(-0.828318, AL::ALValue::array(3, -1.2, 0), AL::ALValue::array(3, 1.65333, 0));
	times[14][6] = 18.24;
	keys[14][6] = AL::ALValue::array(-0.360449, AL::ALValue::array(3, -1.65333, 0), AL::ALValue::array(3, 0, 0));
	...
	times[15][0] = 3.12;
	keys[15][0] = AL::ALValue::array(0.046062, AL::ALValue::array(3, -1.04, 0), AL::ALValue::array(3, 0.8, 0));
	times[15][1] = 5.52;
	...
	times[15][3] = 9.52;
	keys[15][3] = AL::ALValue::array(-0.131882, AL::ALValue::array(3, -0.733333, 0), AL::ALValue::array(3, 0.0533333, 0));
	times[15][4] = 9.68;
	keys[15][4] = AL::ALValue::array(-0.131882, AL::ALValue::array(3, -0.0533333, 0), AL::ALValue::array(3, 1.2, 0));
	times[15][5] = 13.28;
	keys[15][5] = AL::ALValue::array(-0.178554, AL::ALValue::array(3, -1.2, 0), AL::ALValue::array(3, 1.65333, 0));
	times[15][6] = 18.24;
	keys[15][6] = AL::ALValue::array(0.00771189, AL::ALValue::array(3, -1.65333, 0), AL::ALValue::array(3, 0, 0));
	...
	times[16][0] = 3.12;
	keys[16][0] = AL::ALValue::array(1.00328, AL::ALValue::array(3, -1.04, 0), AL::ALValue::array(3, 0.8, 0));
	times[16][1] = 5.52;
	keys[16][1] = AL::ALValue::array(0.308375, AL::ALValue::array(3, -0.8, 0), AL::ALValue::array(3, 0.6, 0));
	times[16][2] = 7.32;
	keys[16][2] = AL::ALValue::array(0.34826, AL::ALValue::array(3, -0.6, 0), AL::ALValue::array(3, 0.733333, 0));
	times[16][3] = 9.52;
	keys[16][3] = AL::ALValue::array(0.257754, AL::ALValue::array(3, -0.733333, 0.0328898), AL::ALValue::array(3, 0.0533333, -0.00239199));
	times[16][4] = 9.68;
	keys[16][4] = AL::ALValue::array(0.242414, AL::ALValue::array(3, -0.0533333, 0.00163625), AL::ALValue::array(3, 0.48, -0.0147262));
	times[16][5] = 11.12;
	keys[16][5] = AL::ALValue::array(0.208666, AL::ALValue::array(3, -0.48, 0), AL::ALValue::array(3, 0.72, 0));
	times[16][6] = 13.28;
	keys[16][6] = AL::ALValue::array(0.231675, AL::ALValue::array(3, -0.72, 0), AL::ALValue::array(3, 0.0133333, 0));
	times[16][7] = 13.32;
	keys[16][7] = AL::ALValue::array(0.231675, AL::ALValue::array(3, -0.0133333, 0), AL::ALValue::array(3, 1.64, 0));
	times[16][8] = 18.24;
	keys[16][8] = AL::ALValue::array(1.00328, AL::ALValue::array(3, -1.64, 0), AL::ALValue::array(3, 0, 0));
	...
	times[17][0] = 3.12;
	keys[17][0] = AL::ALValue::array(1.36522, AL::ALValue::array(3, -1.04, 0), AL::ALValue::array(3, 0.8, 0));
	times[17][1] = 5.52;
	...
	times[17][3] = 9.52;
	keys[17][3] = AL::ALValue::array(-0.11049, AL::ALValue::array(3, -0.733333, 0), AL::ALValue::array(3, 0.0533333, 0));
	times[17][4] = 9.68;
	keys[17][4] = AL::ALValue::array(-0.105888, AL::ALValue::array(3, -0.0533333, -0.000255662), AL::ALValue::array(3, 0.48, 0.00230095));
	times[17][5] = 11.12;
	keys[17][5] = AL::ALValue::array(-0.10282, AL::ALValue::array(3, -0.48, -0.00224985), AL::ALValue::array(3, 0.72, 0.00337477));
	times[17][6] = 13.28;
	keys[17][6] = AL::ALValue::array(-0.0890141, AL::ALValue::array(3, -0.72, 0), AL::ALValue::array(3, 0.0133333, 0));
	times[17][7] = 13.32;
	keys[17][7] = AL::ALValue::array(-0.0890141, AL::ALValue::array(3, -0.0133333, 0), AL::ALValue::array(3, 1.64, 0));
	times[17][8] = 18.24;
	keys[17][8] = AL::ALValue::array(1.33914, AL::ALValue::array(3, -1.64, 0), AL::ALValue::array(3, 0, 0));
	...
	times[18][0] = 3.12;
	keys[18][0] = AL::ALValue::array(0.2464, AL::ALValue::array(3, -1.04, 0), AL::ALValue::array(3, 0.8, 0));
	times[18][1] = 5.52;
	keys[18][1] = AL::ALValue::array(0.9772, AL::ALValue::array(3, -0.8, -0.00906674), AL::ALValue::array(3, 0.6, 0.00680006));
	times[18][2] = 7.32;
	keys[18][2] = AL::ALValue::array(0.984, AL::ALValue::array(3, -0.6, 0), AL::ALValue::array(3, 0.733333, 0));
	times[18][3] = 9.52;
	keys[18][3] = AL::ALValue::array(0.96, AL::ALValue::array(3, -0.733333, 0.024), AL::ALValue::array(3, 0.0533333, -0.00174546));
	times[18][4] = 9.68;
	keys[18][4] = AL::ALValue::array(0.4696, AL::ALValue::array(3, -0.0533333, 0.0228889), AL::ALValue::array(3, 0.48, -0.206));
	times[18][5] = 11.12;
	keys[18][5] = AL::ALValue::array(0.2636, AL::ALValue::array(3, -0.48, 0), AL::ALValue::array(3, 0.72, 0));
	times[18][6] = 13.28;
	keys[18][6] = AL::ALValue::array(0.2712, AL::ALValue::array(3, -0.72, 0), AL::ALValue::array(3, 0.0133333, 0));
	times[18][7] = 13.32;
	keys[18][7] = AL::ALValue::array(0.2712, AL::ALValue::array(3, -0.0133333, 0), AL::ALValue::array(3, 1.64, 0));
	times[18][8] = 18.24;
	keys[18][8] = AL::ALValue::array(0.2592, AL::ALValue::array(3, -1.64, 0), AL::ALValue::array(3, 0, 0));
	...
	times[19][0] = 3.12;
	keys[19][0] = AL::ALValue::array(-0.589097, AL::ALValue::array(3, -1.04, 0), AL::ALValue::array(3, 0.8, 0));
	times[19][1] = 5.52;
	...
	times[19][3] = 9.52;
	keys[19][3] = AL::ALValue::array(-1.29474, AL::ALValue::array(3, -0.733333, 0), AL::ALValue::array(3, 0.0533333, 0));
	times[19][4] = 9.68;
	keys[19][4] = AL::ALValue::array(-1.29474, AL::ALValue::array(3, -0.0533333, 0), AL::ALValue::array(3, 1.2, 0));
	times[19][5] = 13.28;
	keys[19][5] = AL::ALValue::array(-0.713353, AL::ALValue::array(3, -1.2, -0.117846), AL::ALValue::array(3, 1.65333, 0.162365));
	times[19][6] = 18.24;
	keys[19][6] = AL::ALValue::array(-0.454105, AL::ALValue::array(3, -1.65333, 0), AL::ALValue::array(3, 0, 0));
	...
	times[20][0] = 3.12;
	keys[20][0] = AL::ALValue::array(-0.240796, AL::ALValue::array(3, -1.04, 0), AL::ALValue::array(3, 0.8, 0));
	times[20][1] = 5.52;
	...
	times[20][3] = 9.52;
	keys[20][3] = AL::ALValue::array(0.239346, AL::ALValue::array(3, -0.733333, 0), AL::ALValue::array(3, 0.0533333, 0));
	times[20][4] = 9.68;
	keys[20][4] = AL::ALValue::array(0.239346, AL::ALValue::array(3, -0.0533333, 0), AL::ALValue::array(3, 1.2, 0));
	times[20][5] = 13.28;
	keys[20][5] = AL::ALValue::array(-0.033706, AL::ALValue::array(3, -1.2, 0), AL::ALValue::array(3, 1.65333, 0));
	times[20][6] = 18.24;
	keys[20][6] = AL::ALValue::array(-0.00302602, AL::ALValue::array(3, -1.65333, 0), AL::ALValue::array(3, 0, 0));
	...
	times[21][0] = 3.12;
	keys[21][0] = AL::ALValue::array(-0.612024, AL::ALValue::array(3, -1.04, 0), AL::ALValue::array(3, 0.8, 0));
	times[21][1] = 5.52;
	...
	times[21][3] = 9.52;
	keys[21][3] = AL::ALValue::array(-1.13665, AL::ALValue::array(3, -0.733333, 0), AL::ALValue::array(3, 0.0533333, 0));
	times[21][4] = 9.68;
	keys[21][4] = AL::ALValue::array(-1.13665, AL::ALValue::array(3, -0.0533333, 0), AL::ALValue::array(3, 1.2, 0));
	times[21][5] = 13.28;
	keys[21][5] = AL::ALValue::array(-1.12131, AL::ALValue::array(3, -1.2, -0.0153397), AL::ALValue::array(3, 1.65333, 0.0211347));
	times[21][6] = 18.24;
	keys[21][6] = AL::ALValue::array(-0.00762796, AL::ALValue::array(3, -1.65333, 0), AL::ALValue::array(3, 0, 0));
	...
	times[22][0] = 3.12;
	keys[22][0] = AL::ALValue::array(2.11255, AL::ALValue::array(3, -1.04, 0), AL::ALValue::array(3, 0.8, 0));
	times[22][1] = 5.52;
	...
	times[22][3] = 9.52;
	keys[22][3] = AL::ALValue::array(2.11255, AL::ALValue::array(3, -0.733333, 0), AL::ALValue::array(3, 0.0533333, 0));
	times[22][4] = 9.68;
	keys[22][4] = AL::ALValue::array(2.11255, AL::ALValue::array(3, -0.0533333, 0), AL::ALValue::array(3, 1.2, 0));
	times[22][5] = 13.28;
	keys[22][5] = AL::ALValue::array(2.09549, AL::ALValue::array(3, -1.2, 0.0170518), AL::ALValue::array(3, 1.65333, -0.0234936));
	times[22][6] = 18.24;
	keys[22][6] = AL::ALValue::array(0.690342, AL::ALValue::array(3, -1.65333, 0), AL::ALValue::array(3, 0, 0));
	...
	times[23][0] = 3.12;
	keys[23][0] = AL::ALValue::array(1.42206, AL::ALValue::array(3, -1.04, 0), AL::ALValue::array(3, 0.8, 0));
	times[23][1] = 5.52;
	keys[23][1] = AL::ALValue::array(0.668866, AL::ALValue::array(3, -0.8, 0.116583), AL::ALValue::array(3, 0.6, -0.0874375));
	times[23][2] = 7.32;
	keys[23][2] = AL::ALValue::array(0.581429, AL::ALValue::array(3, -0.6, 0.0363558), AL::ALValue::array(3, 0.733333, -0.0444349));
	times[23][3] = 9.52;
	keys[23][3] = AL::ALValue::array(0.426494, AL::ALValue::array(3, -0.733333, 0), AL::ALValue::array(3, 0.0533333, 0));
	times[23][4] = 9.68;
	keys[23][4] = AL::ALValue::array(0.483252, AL::ALValue::array(3, -0.0533333, -0.00187487), AL::ALValue::array(3, 0.48, 0.0168739));
	times[23][5] = 11.12;
	keys[23][5] = AL::ALValue::array(0.500126, AL::ALValue::array(3, -0.48, -0.0049088), AL::ALValue::array(3, 0.72, 0.0073632));
	times[23][6] = 13.28;
	keys[23][6] = AL::ALValue::array(0.520068, AL::ALValue::array(3, -0.72, 0), AL::ALValue::array(3, 0.0133333, 0));
	times[23][7] = 13.32;
	keys[23][7] = AL::ALValue::array(0.520068, AL::ALValue::array(3, -0.0133333, 0), AL::ALValue::array(3, 1.64, 0));
	times[23][8] = 18.24;
	keys[23][8] = AL::ALValue::array(1.39598, AL::ALValue::array(3, -1.64, 0), AL::ALValue::array(3, 0, 0));
	...
	times[24][0] = 3.12;
	keys[24][0] = AL::ALValue::array(-0.276162, AL::ALValue::array(3, -1.04, 0), AL::ALValue::array(3, 0.8, 0));
	times[24][1] = 5.52;
	keys[24][1] = AL::ALValue::array(-0.131966, AL::ALValue::array(3, -0.8, -0.0561005), AL::ALValue::array(3, 0.6, 0.0420754));
	times[24][2] = 7.32;
	keys[24][2] = AL::ALValue::array(0.0183661, AL::ALValue::array(3, -0.6, -0.0628174), AL::ALValue::array(3, 0.733333, 0.0767769));
	times[24][3] = 9.52;
	keys[24][3] = AL::ALValue::array(0.286817, AL::ALValue::array(3, -0.733333, 0), AL::ALValue::array(3, 0.0533333, 0));
	times[24][4] = 9.68;
	keys[24][4] = AL::ALValue::array(0.233125, AL::ALValue::array(3, -0.0533333, 0.00311914), AL::ALValue::array(3, 0.48, -0.0280722));
	times[24][5] = 11.12;
	keys[24][5] = AL::ALValue::array(0.193243, AL::ALValue::array(3, -0.48, 0), AL::ALValue::array(3, 0.72, 0));
	times[24][6] = 13.28;
	keys[24][6] = AL::ALValue::array(0.223922, AL::ALValue::array(3, -0.72, 0), AL::ALValue::array(3, 0.0133333, 0));
	times[24][7] = 13.32;
	keys[24][7] = AL::ALValue::array(0.223922, AL::ALValue::array(3, -0.0133333, 0), AL::ALValue::array(3, 1.64, 0));
	times[24][8] = 18.24;
	keys[24][8] = AL::ALValue::array(-0.27923, AL::ALValue::array(3, -1.64, 0), AL::ALValue::array(3, 0, 0));
	...
	times[25][0] = 3.12;
	keys[25][0] = AL::ALValue::array(-0.0138481, AL::ALValue::array(3, -1.04, 0), AL::ALValue::array(3, 0.8, 0));
	times[25][1] = 5.52;
	...
	times[25][3] = 9.52;
	keys[25][3] = AL::ALValue::array(1.78093, AL::ALValue::array(3, -0.733333, 0), AL::ALValue::array(3, 0.0533333, 0));
	times[25][4] = 9.68;
	keys[25][4] = AL::ALValue::array(1.76252, AL::ALValue::array(3, -0.0533333, 0.00107396), AL::ALValue::array(3, 0.48, -0.00966564));
	times[25][5] = 11.12;
	keys[25][5] = AL::ALValue::array(1.74872, AL::ALValue::array(3, -0.48, 0), AL::ALValue::array(3, 0.72, 0));
	times[25][6] = 13.28;
	keys[25][6] = AL::ALValue::array(1.7702, AL::ALValue::array(3, -0.72, 0), AL::ALValue::array(3, 0.0133333, 0));
	times[25][7] = 13.32;
	keys[25][7] = AL::ALValue::array(1.7702, AL::ALValue::array(3, -0.0133333, 0), AL::ALValue::array(3, 1.64, 0));
	times[25][8] = 18.24;
	keys[25][8] = AL::ALValue::array(0.075124, AL::ALValue::array(3, -1.64, 0), AL::ALValue::array(3, 0, 0));
...
void pickup::throwOutBalls()
{
	...
	times[0][0] = 1.36;
	keys[0][0] = AL::ALValue::array(-0.016916, AL::ALValue::array(3, -0.453333, 0), AL::ALValue::array(3, 0.386667, 0));
	times[0][1] = 2.52;
	keys[0][1] = AL::ALValue::array(-0.6704, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0.386667, 0));
	times[0][2] = 3.68;
	keys[0][2] = AL::ALValue::array(-0.666632, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0, 0));
	...
	times[1][0] = 1.36;
	keys[1][0] = AL::ALValue::array(0.00916195, AL::ALValue::array(3, -0.453333, 0), AL::ALValue::array(3, 0.386667, 0));
	times[1][1] = 2.52;
	keys[1][1] = AL::ALValue::array(0.0106959, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0.386667, 0));
	times[1][2] = 3.68;
	keys[1][2] = AL::ALValue::array(0, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0, 0));
	...
	times[2][0] = 1.36;
	keys[2][0] = AL::ALValue::array(-0.352862, AL::ALValue::array(3, -0.453333, 0), AL::ALValue::array(3, 0.386667, 0));
	times[2][1] = 2.52;
	keys[2][1] = AL::ALValue::array(-0.351328, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0.386667, 0));
	times[2][2] = 3.68;
	keys[2][2] = AL::ALValue::array(-0.352862, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0, 0));
	...
	times[3][0] = 1.36;
	keys[3][0] = AL::ALValue::array(0.00157595, AL::ALValue::array(3, -0.453333, 0), AL::ALValue::array(3, 0.386667, 0));
	times[3][1] = 2.52;
	keys[3][1] = AL::ALValue::array(0.00464392, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0.386667, 0));
	times[3][2] = 3.68;
	keys[3][2] = AL::ALValue::array(0, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0, 0));
	...
	times[4][0] = 1.36;
	keys[4][0] = AL::ALValue::array(-1.0078, AL::ALValue::array(3, -0.453333, 0), AL::ALValue::array(3, 0.386667, 0));
	times[4][1] = 2.52;
	keys[4][1] = AL::ALValue::array(-0.96331, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0.386667, 0));
	times[4][2] = 3.68;
	keys[4][2] = AL::ALValue::array(-0.96331, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0, 0));
	...
	times[5][0] = 1.36;
	keys[5][0] = AL::ALValue::array(-1.33922, AL::ALValue::array(3, -0.453333, 0), AL::ALValue::array(3, 0.386667, 0));
	times[5][1] = 2.52;
	keys[5][1] = AL::ALValue::array(-1.34229, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0.386667, 0));
	times[5][2] = 3.68;
	keys[5][2] = AL::ALValue::array(-1.34229, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0, 0));
	...
	times[6][0] = 1.36;
	keys[6][0] = AL::ALValue::array(0.2624, AL::ALValue::array(3, -0.453333, 0), AL::ALValue::array(3, 0.386667, 0));
	times[6][1] = 2.52;
	keys[6][1] = AL::ALValue::array(0.2488, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0.386667, 0));
	times[6][2] = 3.68;
	keys[6][2] = AL::ALValue::array(0.2488, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0, 0));
	...
	times[7][0] = 1.36;
	keys[7][0] = AL::ALValue::array(-0.447886, AL::ALValue::array(3, -0.453333, 0), AL::ALValue::array(3, 0.386667, 0));
	times[7][1] = 2.52;
	keys[7][1] = AL::ALValue::array(-0.447886, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0.386667, 0));
	times[7][2] = 3.68;
	keys[7][2] = AL::ALValue::array(-0.447886, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0, 0));
	...
	times[8][0] = 1.36;
	keys[8][0] = AL::ALValue::array(0.00310993, AL::ALValue::array(3, -0.453333, 0), AL::ALValue::array(3, 0.386667, 0));
	times[8][1] = 2.52;
	keys[8][1] = AL::ALValue::array(4.19617e-05, AL::ALValue::array(3, -0.386667, 4.19617e-05), AL::ALValue::array(3, 0.386667, -4.19617e-05));
	times[8][2] = 3.68;
	keys[8][2] = AL::ALValue::array(0, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0, 0));
	...
	times[9][0] = 1.36;
	keys[9][0] = AL::ALValue::array(-0.00762796, AL::ALValue::array(3, -0.453333, 0), AL::ALValue::array(3, 0.386667, 0));
	times[9][1] = 2.52;
	keys[9][1] = AL::ALValue::array(-0.00455999, AL::ALValue::array(3, -0.386667, -0.00127133), AL::ALValue::array(3, 0.386667, 0.00127133));
	times[9][2] = 3.68;
	keys[9][2] = AL::ALValue::array(0, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0, 0));
	...
	times[10][0] = 1.36;
	keys[10][0] = AL::ALValue::array(0.70253, AL::ALValue::array(3, -0.453333, 0), AL::ALValue::array(3, 0.386667, 0));
	times[10][1] = 2.52;
	keys[10][1] = AL::ALValue::array(0.704064, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0.386667, 0));
	times[10][2] = 3.68;
	keys[10][2] = AL::ALValue::array(0.70253, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0, 0));
	...
	times[11][0] = 1.36;
	keys[11][0] = AL::ALValue::array(1.38363, AL::ALValue::array(3, -0.453333, 0), AL::ALValue::array(3, 0.386667, 0));
	times[11][1] = 2.52;
	keys[11][1] = AL::ALValue::array(1.44192, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0.386667, 0));
	times[11][2] = 3.68;
	keys[11][2] = AL::ALValue::array(1.44192, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0, 0));
	...
	times[12][0] = 1.36;
	keys[12][0] = AL::ALValue::array(0.294486, AL::ALValue::array(3, -0.453333, 0), AL::ALValue::array(3, 0.386667, 0));
	times[12][1] = 2.52;
	keys[12][1] = AL::ALValue::array(0.196309, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0.386667, 0));
	times[12][2] = 3.68;
	keys[12][2] = AL::ALValue::array(0.196309, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0, 0));
	...
	times[13][0] = 1.36;
	keys[13][0] = AL::ALValue::array(-0.0537319, AL::ALValue::array(3, -0.453333, 0), AL::ALValue::array(3, 0.386667, 0));
	times[13][1] = 2.52;
	keys[13][1] = AL::ALValue::array(-0.023052, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0.386667, 0));
	times[13][2] = 3.68;
	keys[13][2] = AL::ALValue::array(-0.023052, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0, 0));
	...
	times[14][0] = 1.36;
	keys[14][0] = AL::ALValue::array(-0.360449, AL::ALValue::array(3, -0.453333, 0), AL::ALValue::array(3, 0.386667, 0));
	times[14][1] = 2.52;
	keys[14][1] = AL::ALValue::array(-0.363515, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0.386667, 0));
	times[14][2] = 3.68;
	keys[14][2] = AL::ALValue::array(-0.360423, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0, 0));
	...
	times[15][0] = 1.36;
	keys[15][0] = AL::ALValue::array(0.00771189, AL::ALValue::array(3, -0.453333, 0), AL::ALValue::array(3, 0.386667, 0));
	times[15][1] = 2.52;
	keys[15][1] = AL::ALValue::array(0.00464392, AL::ALValue::array(3, -0.386667, 0.00128532), AL::ALValue::array(3, 0.386667, -0.00128532));
	times[15][2] = 3.68;
	keys[15][2] = AL::ALValue::array(0, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0, 0));
	...
	times[16][0] = 1.36;
	keys[16][0] = AL::ALValue::array(1.00328, AL::ALValue::array(3, -0.453333, 0), AL::ALValue::array(3, 0.386667, 0));
	times[16][1] = 2.52;
	keys[16][1] = AL::ALValue::array(0.0349066, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0.386667, 0));
	times[16][2] = 3.68;
	keys[16][2] = AL::ALValue::array(0.0349066, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0, 0));
	...
	times[17][0] = 1.36;
	keys[17][0] = AL::ALValue::array(1.33914, AL::ALValue::array(3, -0.453333, 0), AL::ALValue::array(3, 0.386667, 0));
	times[17][1] = 2.52;
	keys[17][1] = AL::ALValue::array(1.51248, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0.386667, 0));
	times[17][2] = 3.68;
	keys[17][2] = AL::ALValue::array(1.51248, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0, 0));
	...
	times[18][0] = 1.36;
	keys[18][0] = AL::ALValue::array(0.2592, AL::ALValue::array(3, -0.453333, 0), AL::ALValue::array(3, 0.386667, 0));
	times[18][1] = 2.52;
	keys[18][1] = AL::ALValue::array(0.2356, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0.386667, 0));
	times[18][2] = 3.68;
	keys[18][2] = AL::ALValue::array(0.9904, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0, 0));
	...
	times[19][0] = 1.36;
	keys[19][0] = AL::ALValue::array(-0.454105, AL::ALValue::array(3, -0.453333, 0), AL::ALValue::array(3, 0.386667, 0));
	times[19][1] = 2.52;
	keys[19][1] = AL::ALValue::array(-0.449504, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0.386667, 0));
	times[19][2] = 3.68;
	keys[19][2] = AL::ALValue::array(-0.454105, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0, 0));
	...
	times[20][0] = 1.36;
	keys[20][0] = AL::ALValue::array(-0.00302602, AL::ALValue::array(3, -0.453333, 0), AL::ALValue::array(3, 0.386667, 0));
	times[20][1] = 2.52;
	keys[20][1] = AL::ALValue::array(0.00617791, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0.386667, 0));
	times[20][2] = 3.68;
	keys[20][2] = AL::ALValue::array(0, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0, 0));
	...
	times[21][0] = 1.36;
	keys[21][0] = AL::ALValue::array(-0.00762796, AL::ALValue::array(3, -0.453333, 0), AL::ALValue::array(3, 0.386667, 0));
	times[21][1] = 2.52;
	keys[21][1] = AL::ALValue::array(-0.00455999, AL::ALValue::array(3, -0.386667, -0.00127133), AL::ALValue::array(3, 0.386667, 0.00127133));
	times[21][2] = 3.68;
	keys[21][2] = AL::ALValue::array(0, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0, 0));
	...
	times[22][0] = 1.36;
	keys[22][0] = AL::ALValue::array(0.690342, AL::ALValue::array(3, -0.453333, 0), AL::ALValue::array(3, 0.386667, 0));
	times[22][1] = 2.52;
	keys[22][1] = AL::ALValue::array(0.690342, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0.386667, 0));
	times[22][2] = 3.68;
	keys[22][2] = AL::ALValue::array(0.690342, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0, 0));
	...
	times[23][0] = 1.36;
	keys[23][0] = AL::ALValue::array(1.39598, AL::ALValue::array(3, -0.453333, 0), AL::ALValue::array(3, 0.386667, 0));
	times[23][1] = 2.52;
	keys[23][1] = AL::ALValue::array(0.0107799, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0.386667, 0));
	times[23][2] = 3.68;
	keys[23][2] = AL::ALValue::array(0.0107799, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0, 0));
	...
	times[24][0] = 1.36;
	keys[24][0] = AL::ALValue::array(-0.27923, AL::ALValue::array(3, -0.453333, 0), AL::ALValue::array(3, 0.386667, 0));
	times[24][1] = 2.52;
	keys[24][1] = AL::ALValue::array(0.305223, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0.386667, 0));
	times[24][2] = 3.68;
	keys[24][2] = AL::ALValue::array(0.305223, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0, 0));
	...
	times[25][0] = 1.36;
	keys[25][0] = AL::ALValue::array(0.075124, AL::ALValue::array(3, -0.453333, 0), AL::ALValue::array(3, 0.386667, 0));
	times[25][1] = 2.52;
	keys[25][1] = AL::ALValue::array(0.0873961, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0.386667, 0));
	times[25][2] = 3.68;
	keys[25][2] = AL::ALValue::array(0.0873961, AL::ALValue::array(3, -0.386667, 0), AL::ALValue::array(3, 0, 0));
